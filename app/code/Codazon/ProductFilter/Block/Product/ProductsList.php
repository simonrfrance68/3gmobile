...
class ProductsList extends \Magento\CatalogWidget\Block\Product\ProductsList
{
    ...
    public function getImageHtml($product, $width, $height, $imageHelper)
    {
        if ($this->productLazyImg) {
            ...
            $mainImage = '<span class="main-image"><span class="product-image-container"><span class="product-image-wrapper"><img data-hasoptions=\''.($product->getHasOptions()? '1':'0').'\' class="product-image-photo main-img" data-lazysrc="'.$mainImage->resize($width, $height)->getUrl().'" alt="'. ($label = $this->escapeHtmlAttr($mainImage->getLabel())) .'" /></span></span></span>';
            $hoveredImage = $imageHelper->init($product, 'category_page_grid')->setImageFile($product->getData('thumbnail'));
            $hoveredImage = '<span class="hover-image"><span class="product-image-container"><span class="product-image-wrapper"><img class="product-image-photo hovered-img" data-lazysrc="'.$hoveredImage->resize($width, $height)->getUrl().'" alt="'.$label.'" /></span></span></span>';
        } else {
            $mainImage = $imageHelper->init($product, 'category_page_grid')->setImageFile($product->getData('small_image'));
            $mainImage = '<span class="main-image"><span class="product-image-container"><span class="product-image-wrapper"><img data-hasoptions=\''.($product->getHasOptions()? '1':'0').'\' class="product-image-photo main-img" src="'.$mainImage->resize($width, $height)->getUrl().'" alt="'. ($label = $this->escapeHtmlAttr($mainImage->getLabel())) .'" /></span></span></span>';
            $hoveredImage = $imageHelper->init($product, 'category_page_grid')->setImageFile($product->getData('thumbnail'));
            $hoveredImage = '<span class="hover-image"><span class="product-image-container" style="width:'.$width.'px !important;height: '.$height.'px !important;"><span class="product-image-wrapper"><img class="product-image-photo hovered-img" src="'.$hoveredImage->resize($width, $height)->getUrl().'" alt="'.$label.'"/></span></span></span>';
        }
