...
class Save extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        //clone blocks.csv
        $rows = $this->csvReader->getData($dsc.'/blocks.csv');
        $header = array_shift($rows);

        $fp = fopen($dsc.'/blocks.csv', 'w');
        fputcsv($fp, $header);
        foreach ($rows as $row) {
            $data = $row;
            $data[0] = str_replace($find1, $replace1, $data[0]);
            $data[0] = str_replace($find2, $replace2, $data[0]);
            $data[0] = ucwords($data[0]);
            $data[1] = str_replace($find1, $replace1, $data[1]);
            $data[1] = str_replace($find2, $replace2, $data[1]);
            fputcsv($fp, $data);
        }
        fclose($fp);
        //end clone
