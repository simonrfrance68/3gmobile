 ...
 * @package Geissweb\Euvat\Model\Rewrite
 */
class TaxCalculation extends \Magento\Tax\Model\Calculation
{
    ...
    protected function getRateOriginRequest($store = null)
    {
        ...
        if ($logger instanceof \Geissweb\Euvat\Logger\Logger
            ...
            $logger->debug("[Rewrite\TaxCalculation] getRateOriginRequest country is ".$request->getCountryId());
            $logger->debug("[Rewrite\TaxCalculation] currentTaxCountryRegistry country is ".$currentTaxCountryRegistry->get());
        } else {
            return $request;
        }
        ...
        if ($currentTaxCountryRegistry->get() !== null
            && $config->getEnableTresholdCountries()
            ...
            if ($logger instanceof \Geissweb\Euvat\Logger\Logger) {
                $logger->debug("[Rewrite\TaxCalculation] getRateOriginRequest CBT enabled and threshold country: ".$currentTaxCountryRegistry->get());
            }
