...
(function (factory) {
    if (typeof define === 'function' && define.amd) {
        define([
            ...
            "MageWorx_SeoXTemplates/mageworx/jstree/jquery.hotkeys"
        ], factory);
    } else {
        factory(jQuery);
    }
        ...
        (function ($) {
            $.vakata.xslt = function (xml, xsl, callback) {
                ...
                if(typeof window.DOMParser !== "undefined" && typeof window.XMLHttpRequest !== "undefined" && typeof window.XSLTProcessor !== "undefined") {
                    ...
                    if($.isFunction(processor.transformDocument)) {
                        ...
                        callback.call(null, $("<div></div>").append(rs).html());
                        return true;
                    }
                }
                return false;
            ...
            var xsl = {
                    'nest' : '<' + '?xml version="1.0" encoding="utf-8" ?>' +
                        ...
                        '<xsl:output method="html" encoding="utf-8" omit-xml-declaration="yes" standalone="no" indent="no" media-type="text/html"></xsl:output>' +
                        '<xsl:template match="/">' +
                        '	<xsl:call-template name="nodes">' +
                        '		<xsl:with-param name="node" select="/root"></xsl:with-param>' +
                        '	</xsl:call-template>' +
                        '</xsl:template>' +
                        ...
                        '		<xsl:variable name="children" select="count(./item) &gt; 0"></xsl:variable>' +
                        '		<li>' +
                        ...
                        '					<xsl:choose>' +
                        '					<xsl:when test="@href"><xsl:value-of select="@href"></xsl:value-of></xsl:when>' +
                        '					<xsl:otherwise>#</xsl:otherwise>' +
                        '					</xsl:choose>' +
                        '				</xsl:attribute>' +
                        '				<xsl:attribute name="class"><xsl:value-of select="@lang"></xsl:value-of> <xsl:value-of select="@class"></xsl:value-of></xsl:attribute>' +
                        ...
                        '							<xsl:if test="string-length(attribute::icon) > 0 and not(contains(@icon,\'/\'))"><xsl:value-of select="@icon"></xsl:value-of></xsl:if>' +
                        '						</xsl:attribute>' +
                        '						<xsl:if test="string-length(attribute::icon) > 0 and contains(@icon,\'/\')"><xsl:attribute name="style">background:url(<xsl:value-of select="@icon"></xsl:value-of>) center center no-repeat;</xsl:attribute></xsl:if>' +
                        '						<xsl:text>&#xa0;</xsl:text>' +
                        '					</ins>' +
                        '					<xsl:copy-of select="./child::node()"></xsl:copy-of>' +
                        '				</a>' +
                        '			</xsl:for-each>' +
                        '			<xsl:if test="$children or @hasChildren"><xsl:call-template name="nodes"><xsl:with-param name="node" select="current()"></xsl:with-param></xsl:call-template></xsl:if>' +
                        '		</li>' +
                        '	</xsl:for-each>' +
                        '	</ul>' +
                        '</xsl:template>' +
                    ...
                    'flat' : '<' + '?xml version="1.0" encoding="utf-8" ?>' +
                        '<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >' +
                        '<xsl:output method="html" encoding="utf-8" omit-xml-declaration="yes" standalone="no" indent="no" media-type="text/xml"></xsl:output>' +
                        '<xsl:template match="/">' +
                        '	<ul>' +
                        '	<xsl:for-each select="//item[not(@parent_id) or @parent_id=0 or not(@parent_id = //item/@id)]">' + /* the last `or` may be removed */
                        '		<xsl:call-template name="nodes">' +
                        '			<xsl:with-param name="node" select="."></xsl:with-param>' +
                        '			<xsl:with-param name="is_last" select="number(position() = last())"></xsl:with-param>' +
                        '		</xsl:call-template>' +
                        '	</xsl:for-each>' +
                        '	</ul>' +
                        '</xsl:template>' +
                        '<xsl:template name="nodes">' +
                        '	<xsl:param name="node"></xsl:param>' +
                        '	<xsl:param name="is_last"></xsl:param>' +
                        '	<xsl:variable name="children" select="count(//item[@parent_id=$node/attribute::id]) &gt; 0"></xsl:variable>' +
                        '	<li>' +
                        '	<xsl:attribute name="class">' +
