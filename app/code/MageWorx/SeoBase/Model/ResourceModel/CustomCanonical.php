...
class CustomCanonical extends AbstractDb
{
    ...
    {
        if ($entityType == Rewrite::ENTITY_TYPE_PRODUCT) {
            $status = $this->resourceProduct->getAttributeRawValue($entityId, 'status', $storeId);

            if ($status == SourceProductStatus::STATUS_ENABLED) {
                $visibility = $this->resourceProduct->getAttributeRawValue($entityId, 'visibility', $storeId);

                return in_array($visibility, $this->productVisibility->getVisibleInSiteIds());
            }
